...
def parse_requirements(file_path=ROOT.parent / 'requirements.txt', package=''):
    ...
    Args:
        ...
        package (str, optional): Python package to use instead of requirements.txt file, i.e. package='core'.
    ...
    Example:
        ```python
        from core.utils.checks import parse_requirements

        parse_requirements(package='core')
        ```
    """
...
def check_version(current: str = '0.0.0',
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='core'
        try:
...
def check_latest_pypi_version(package_name='core'):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the core package is available on PyPI.
        ...
        with contextlib.suppress(Exception):
            ...
            if check_version(__version__, f'<{latest}'):  # check if current version is < latest version
                LOGGER.info(f'New https://pypi.org/project/ultralytics/{latest} available 😃 '
                            f"Update with 'pip install -U core'")
                return True
    return False
...
def check_requirements(requirements=ROOT.parent / 'requirements.txt', exclude=(), install=True, cmds=''):
    ...
    Example:
        ```python
        from core.utils.checks import check_requirements

        # Check a requirements.txt file
        check_requirements('path/to/requirements.txt')

        # Check a single package
        check_requirements('core>=8.0.0')

        # Check multiple packages
        check_requirements(['numpy', 'core>=8.0.0'])
        ```
    """
...
def collect_system_info():
    ...
    for r in parse_requirements(package='core'):
        try:
...
def check_amp(model):
    ...
    Example:
        ...
        from core.utils.checks import check_amp
