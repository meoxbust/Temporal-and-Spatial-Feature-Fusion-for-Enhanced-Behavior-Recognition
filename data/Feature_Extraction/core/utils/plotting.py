...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Arial.ttf', pil=False, example='abc'):
        ...
        # self.lw = line_width or max(round(sum(im.shape) / 2), 1)  # line width
        if self.pil:  # use PIL
...
def plot_labels(boxes, cls, names=(), save_dir=Path(''), on_plot=None):
    ...
    for cls, box in zip(cls[:500], boxes[:500]):
        ImageDraw.Draw(img).rectangle(box, width=0.5, outline=colors(cls))  # plot
    ax[1].imshow(img)
    ax[1].axis('off')
...
def save_one_box(xyxy, im, file=Path('im.jpg'), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    Example:
        ```python
        from core.utils.plotting import save_one_box
...
def plot_results(file='path/to/results.csv', dir='', segment=False, pose=False, classify=False, on_plot=None):
    ...
    Example:
        ```python
        from core.utils.plotting import plot_results
