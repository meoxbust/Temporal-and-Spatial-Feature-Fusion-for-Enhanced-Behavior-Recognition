...
import pandas as pd
import os
from core.engine.results import Results
from core.utils import DEFAULT_CFG, ops
from core.models.yolo.detect.predict import DetectionPredictor
...
os.mkdir(f"rgb_images/{src_name}")
...
while cap.isOpened():
    ...
    if success:
        ...
        if frame_count == 1:
            for i in range(n):
                os.mkdir(f"rgb_images/{src_name}/id_{i+1}")
        ...
        for idx in range(n):
            ...
            if frame_count == 1:
                ...
                for i in range(len(compare_window)):
                    ...
                    if iou_score > max_iou_score[i] and iou_score >= 0.5:
        ...
        for value in compare_window:
            output_rgb = f"rgb_images/{src_name}/id_{value[0]}/{src_name}_rgb_{value[0]}_{value[4]}.png"
